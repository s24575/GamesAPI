create table company(id integer not null auto_increment, source_id integer not null, country integer, description TEXT, name varchar(255), url varchar(255), cover_id integer, primary key(id));
create table cover(id integer not null auto_increment, source_id integer not null, height integer, width integer, image_id varchar(255), url varchar(255), primary key(id));
...
create table game_language(game_id integer not null, language_id integer not null);
create table game_platform(game_id integer not null, platform_id integer not null);
create table game_game(game1_id integer not null, game2_id integer not null);
...
ALTER TABLE game_game ADD FOREIGN KEY (game1_id) REFERENCES game(id);
ALTER TABLE game_game ADD FOREIGN KEY (game2_id) REFERENCES game(id);
